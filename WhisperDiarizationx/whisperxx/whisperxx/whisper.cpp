...
const char* whisper_print_timings(struct whisper_context * ctx) {
    ...
    snprintf(buffer + strlen(buffer), sizeof(buffer) - strlen(buffer), "%s:     load time = %8.2f ms\n", "\n", __func__, ctx->t_load_us / 1000.0f);
    if (ctx->state != nullptr) {
        const int32_t n_sample = std::max(1, ctx->state->n_sample);
        const int32_t n_encode = std::max(1, ctx->state->n_encode);
        const int32_t n_decode = std::max(1, ctx->state->n_decode);
        
        snprintf(buffer + strlen(buffer), sizeof(buffer) - strlen(buffer), "%s:     fallbacks = %3d p / %3d h\n", __func__, ctx->state->n_fail_p, ctx->state->n_fail_h);
        snprintf(buffer + strlen(buffer), sizeof(buffer) - strlen(buffer), "%s:      mel time = %8.2f ms\n", __func__, ctx->state->t_mel_us / 1000.0f);
        snprintf(buffer + strlen(buffer), sizeof(buffer) - strlen(buffer), "%s:   sample time = %8.2f ms / %5d runs (%8.2f ms per run)\n", __func__, 1e-3f * ctx->state->t_sample_us, n_sample, 1e-3f * ctx->state->t_sample_us / n_sample);
        snprintf(buffer + strlen(buffer), sizeof(buffer) - strlen(buffer), "%s:   encode time = %8.2f ms / %5d runs (%8.2f ms per run)\n", __func__, 1e-3f * ctx->state->t_encode_us, n_encode, 1e-3f * ctx->state->t_encode_us / n_encode);
        snprintf(buffer + strlen(buffer), sizeof(buffer) - strlen(buffer), "%s:   decode time = %8.2f ms / %5d runs (%8.2f ms per run)\n", __func__, 1e-3f * ctx->state->t_decode_us, n_decode, 1e-3f * ctx->state->t_decode_us / n_decode);
    }
    
    snprintf(buffer + strlen(buffer), sizeof(buffer) - strlen(buffer), "%s:    total time = %8.2f ms\n", __func__, (t_end_us - ctx->t_start_us)/1000.0f);
